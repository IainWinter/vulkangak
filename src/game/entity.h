...
// Last system stored the bindings in a map inside each entity
// but this could have just been a layout of memory offsets and stored in the list itself

// map each component type to an id

inline static int s_nextComponentId = 0;
...
class Entity {
public:
    int id; // this will get assigned by the list
            // keep it public for ezpz access
    
    EntityLayout* layout; // same as this

    // part of the list api needs the concrete class to provide a function with the signature 'static EntityLayout getLayout()'
    
    // I see so each entity stored the binding so there could be a get/has function
    // just fill it out when added to the list like the id

    template<typename _t>
    _t& get() {
        for (const EntityLayout::Component& c : layout->components) {
            if (c.componentId == getComponentId<_t>()) {
                return *(_t*)((char*)this + c.offset);
            }
        }
...
public:
    ...
    static EntityLayout getLayout() {
        EntityLayout layout;
        layout.components.push_back({ getComponentId<int>(), offsetof(TestEntity, x) });
        return layout;
    }
};
...
public:
    ...
    // copies the entity into the list, return the id
    int add(const _t& entity) {

    }

private:
    std::vector<_t> entities;
...
// of the archetype every time reg/move is called
template<typename _t>
